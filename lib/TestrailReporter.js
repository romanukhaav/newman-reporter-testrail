...
class TestRailReporter {
  constructor(emitter, reporterOptions, options) {
    emitter.on('beforeDone', (err, args) => {
		...
		this.domain = reporterOptions.domain || process.env.TESTRAIL_DOMAIN;
		this.host = reporterOptions.host || process.env.TESTRAIL_HOST;
		this.username = reporterOptions.username || process.env.TESTRAIL_USERNAME;
		this.apikey = reporterOptions.password || process.env.TESTRAIL_APIKEY;
		this.projectId = reporterOptions.projectId || process.env.TESTRAIL_PROJECTID;
		this.suiteId = reporterOptions.suiteId || process.env.TESTRAIL_SUITEID;
		this.version = reporterOptions.version ||process.env.TESTRAIL_VERSION;
		this.includeAll = reporterOptions.includeAll || process.env.TESTRAIL_INCLUDEALL;
		this.runId = reporterOptions.runId || process.env.TESTRAIL_RUNID;
		this.title = reporterOptions.title || process.env.TESTRAIL_TITLE;
		this.milestoneId = reporterOptions.milestoneId || process.env.TESTRAIL_MILESTONEID;
		this.verbose = reporterOptions.verbose || process.env.VERBOSE || 'true';
		this.logging = reporterOptions.logging || process.env.TESTRAIL_LOGGING || 'full';
        ...
        while (i < strings.length && strings[i].match(testCaseRegex)) {
          ...
          if (assertion.skipped === true) {
            ...
            if (assertion.error) {
			  ...
			  if (this.logging.toLowerCase() === 'errors') {
				  currentResult.comment = currentResult.comment.concat(
					`\n${log.url}`,
				  );
				  currentResult.comment = currentResult.comment.concat(
            ...
            if (
              ...
              this.logging.toLowerCase() !== 'none' && this.logging.toLowerCase() !== 'errors'
            ) {
              currentResult.comment = currentResult.comment.concat(
                `\n${log.url}`,
              );
  ...
  pushToTestrail() {
	...
	if (!this.host)
      ...
      if (!this.title) {
        ...
        response = request(
          'GET',
          `${this.host}/index.php?/api/v2/${path}`,
          { headers },
        );
      ...
      if (this.runId) {
        ...
        if (this.runId.toLowerCase() === 'latest') {
          response = request(
            'GET',
            `${this.host}/index.php?/api/v2/get_runs/${this.projectId}`,
            { headers },
          );
        ...
        response = request(
          'GET',
          `${this.host}/index.php?/api/v2/get_run/${this.runId}`,
          { headers },
        );
      } else {
        ...
        response = request(
          'POST',
          `${this.host}/index.php?/api/v2/add_run/${this.projectId}`,
          {
            headers,
            json,
          },
      ...
      response = request(
        'POST',
        `${this.host}/index.php?/api/v2/add_results_for_cases/${this.runId}`,
        {
          headers,
          json: {
